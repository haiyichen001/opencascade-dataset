...
# 按顺序执行
python scripts/gen_step_cpu.py          # STEP — CadQuery + cq_warehouse + cq_gears
python scripts/gen_step_to_stl_cpu.py   # STEP → STL — CadQuery tessellation
python scripts/gen_stl_to_ply_cpu.py    # STL → PLY — trimesh 表面采样 16384 点
python scripts/gen_step_to_npy_cpu.py   # STEP → NPY — OCC BRepClass3d_SolidClassifier
python scripts/gen_png.py               # PNG — OCC 离屏渲染 24 视角
```
